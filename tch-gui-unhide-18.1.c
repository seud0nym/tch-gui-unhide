#!/bin/sh
echo 000@$(date +%H:%M:%S): Built for firmware version 18.1.c - Release 2021.06.26
...
if [ $THEME_ONLY = n ]; then
  ...
  do
    ...
    sed -e "s/\(current_year); ngx.print('\)/\1 \[tch-gui-unhide 2021.06.26 for FW Version 18.1.c ($MKTING_VERSION)\]/" -i $l
  done
fi
