...
ip6tables -t mangle -S PREROUTING | grep "No-Hijack-DHCP-DNSv6" | while read action params; do
...
ip6tables -t mangle -S PREROUTING | grep '!tproxy-go/dnsv6@.*-j TPROXY' | sed -e 's/-m comment.*$//' | while read action params; do 
  ip6tables -t mangle -I $params  -m comment --comment "No-Hijack-DHCP-DNSv6" -j LOG --log-prefix '** HIJACKED ** ' 
done

# Create the rules to always accept these DNS servers
for dns in $_DNS6; do
  for proto in udp tcp; do
    ip6tables -t mangle -I PREROUTING -d $dns/128 -p $proto -m $proto --dport 53 -m comment --comment "No-Hijack-DHCP-DNSv6" -j ACCEPT 2>/dev/null
  done
done
