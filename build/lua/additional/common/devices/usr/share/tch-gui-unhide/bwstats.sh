...
if [ "/$1/" = "/show/" ]; then
  ...
  if [ "/$1/" = "/disable/" -o "/$(uci -q get bwstats.config.enabled)/" != "/1/" ]; then
    ...
    for CMD in iptables ip6tables; do
      ...
      $CMD -t mangle -C FORWARD -i br-lan -j BWSTATSRX 2>/dev/null || $CMD -t mangle -I FORWARD 1 -i br-lan -j BWSTATSRX
      $CMD -t mangle -nL BWSTATSTX >/dev/null 2>&1 || $CMD -t mangle -N BWSTATSTX
      $CMD -t mangle -C FORWARD -o br-lan -j BWSTATSTX 2>/dev/null || $CMD -t mangle -I FORWARD 1 -o br-lan -j BWSTATSTX
    done
